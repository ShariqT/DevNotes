...
def start():
    ...
    res.headers['Content-Security-Policy'] = "default-src 'self' appssdk.zoom.us; style-src 'self'; script-src 'self' appssdk.zoom.us; img-src 'self'; connect-src 'self'; base-uri 'self'; form-action 'self'"
    return res
